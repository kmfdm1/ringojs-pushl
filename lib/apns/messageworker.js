...
function init() {
    ...
    status.manager = new PushManager(
            ...
            mdist.namespace);
    status.manager.registerFailedConnectionListener(rejectedNotificationListener);
    status.manager.registerExpiredTokenListener(expiredTokenListener);
    status.manager.requestExpiredTokens(status.manager);
};

var requestExpiredTokens = function(manager) {
    manager.requestExpiredTokens();
    status.feedbackTimeoutId = setTimeout(requestExpiredTokens, 86400000, status.manager);
};
...
function process(el) {
    ...
    var ttl = el.getTtl();
    for each (var recipient in recipients) try {
        status.manager.getQueue().put(new SimpleApnsPushNotification(TokenUtil.tokenStringToByteArry(recipient), pl));
    } catch (e) {
        log.error("failed", e);
    }
};
