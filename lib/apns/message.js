...
var {ApnsPayloadBuilder} = com.relayrides.pushy.apns;
...
var Message = exports.Message = function(data, options) {
    ...
    if (data instanceof ApnsPayloadBuilder) {
        ...
        this.payload = new ApnPayloadBuilder();
        if (data && typeof(data) != "object") {
            this.payload.setAlertBody("message", data);
        } else if (data) {
            for (let prop in data) {
                switch (prop) {
                case "message":
                    this.payload.setAlertBody(data.alert.body);
                    break;
                case "shortmessage":
                    this.payload.setAlertTitle(data.alert.title); // title only shows on apple-watch atm
                    break;
                case "badgenumber":
                    this.setBadgeNumber(parseInt(data.badgenumber,10));
                    break;
                case "category":
                    this.setCategoryName(data.category);
                    break;
                case "launchImageName":
                    this.setLaunchImageFileName(data.launchImageName);
                    break;
                case "localizedActionButtonKey":
                    this.setLocalizedActionButtonKey(data.localizedActionButtonKey);
                    break;
                case "localizedAlert":
                    // FIXME: we just take the arguments as given. check to avoid error
                    var la = data.localizedAlert;
                    if (typeof(la) != "object" || !la.message || !la.messageArgs || !la.title || !la.titleArgs) {
                        throw new Error("Localized alert must be an object containing the message + arguments and the title");
                    }
                    this.setLocalizedAlertMessage(la.message, la.messageArgs);
                    this.setLocalizedAlertTitle(la.title, la.titleArgs);
                    break;
                case "soundfileName":
                    this.setSoundFileName(data.soundfileName);
                    break;
                case "showActionButton":
                    this.setShowActionButton(toBoolen(data.showActionButton));
                    break;
                default:
                    this.setCustomProperty(prop, data[prop]);
                    break;
                }
            }
        }
