...
var {APNSMessage} = require("../lib/apns/message");
...
exports.testCreate = function() {
    ...
    var m = new APNSMessage(messageData, messages.withRecipients);
    assert.equal(m.messageObject.data, messageData);
    ...
    var m = new GCMMessage(messageData, messages.withDelayWhileIdle);
    assert.equal(m.messageObject.data, messageData);
    assert.equal(m.recipients[0], undefined);
    assert.equal(m.ttl, undefined);
    assert.equal(m.retries, undefined);
    ...
    assert.equal(m.delay, 0);
    var m = new GCMMessage(messageData, messages.withDryRun);
    assert.equal(m.messageObject.data, messageData);
    assert.equal(m.recipients[0], undefined);
    assert.equal(m.ttl, undefined);
    assert.equal(m.retries, undefined);
    ...
    assert.equal(m.messageObject.delay_while_idle, undefined);
    assert.equal(m.delay, 0);
    var m = new GCMMessage(messageData, messages.withTimeToLive);
    assert.equal(m.messageObject.data, messageData);
    assert.equal(m.recipients[0], undefined);
    ...
    assert.equal(m.messageObject.delay_while_idle, undefined);
    assert.equal(m.delay, 0);
    var m = new GCMMessage(messageData, messages.withDelay);
    assert.equal(m.messageObject.data, messageData);
    assert.equal(m.recipients[0], undefined);
    assert.equal(m.ttl, undefined);
    assert.equal(m.retries, undefined);
...
exports.testAddRecipients = function() {
    var m = new GCMMessage(messageData, messages.normal);
...
exports.testGetSendableObjects = function() {
    var m = new GCMMessage(messageData, messages.withAll);
    var ttl = m.getTtl();
    ...
    assert.deepEqual(sendables[0], {
        ...
        dry_run: true,
        time_to_live: ttl,
        recipient_ids: messages.withAll.recipients
    });
    var m = new GCMMessage(messageData, messages.with1001);
    var ttl = m.getTtl();
    var sendables = m.getSendableObjects();
    ...
    assert.deepEqual(sendables[0], {
        ...
        dry_run: true,
        time_to_live: ttl,
        recipient_ids: rids.slice(0, 1000)
    });
